...
const nav=(active)=>`<aside class="side"><div class="logo"><span>◆</span> SlideGuard</div><div class="nav">${['首页','扫描设置','敏感词库','扫描结果','问题详情','修复','修复结果'].map(x=>`<div class="${x===active?'active':''}">${x}</div>`).join('')}</div><div class="privacy"><b>● 离线运行</b>文件仅在本机处理，<br>不会上传或访问互联网</div></aside>`;
...
/* 04B ── 扫描结果概览 */
'4b':()=>shell('扫描结果',`${heading('扫描结果概览','年度经营汇报.pptx · 标准检查 · builtin-rules-v1.0','<button class="btn" onclick="location.hash=\'2\'">重新扫描</button><button class="btn primary" onclick="location.hash=\'4c\'">查看问题</button>')}<div class="grid summary">${[['S1 严重','3','s1'],['S2 高风险','5','s2'],['S3 一般','18','s3'],['S4 建议','2','s4'],['涉及页面','16',''],['可自动修复','11','ok']].map(x=>`<div class="card stat"><span class="badge ${x[2]}">${x[0]}</span><strong>${x[1]}</strong></div>`).join('')}</div><div class="card charts" style="margin-top:20px"><h2>按问题类型统计</h2>${[['敏感词',92],['字体规范',78],['文本溢出',61],['字号规范',52],['元素对齐',38]].map(x=>`<div class="bar"><label>${x[0]}</label><i style="width:${x[1]}%"></i></div>`).join('')}</div><div class="info" style="margin-top:18px;color:#9b5b00;border-color:#f2c879;background:#fff9e8"><b>扫描未完成</b>　可查看已发现问题，但自动修复不可用。</div>`),

/* 04C ── 问题列表 */
'4c':()=>shell('扫描结果',`${heading('问题列表','共 28 个问题','<button class="btn" onclick="location.hash=\'4b\'">返回概览</button>')}<div class="grid filters"><div class="input">搜索问题描述　⌕</div>${['页面','问题类型','严重级别','可自动修复','处理状态'].map(x=>`<div class="input">${x}　⌄</div>`).join('')}</div><div class="card"><table class="table"><thead><tr><th>选择</th><th>页面</th><th>问题类型</th><th>严重级别</th><th>问题描述</th><th>可自动修复</th><th>处理状态</th></tr></thead><tbody>${[['3','敏感词','s1','检测到敏感词：内部代号','否'],['5','字体规范','s1','标题字体不是微软雅黑','是'],['9','标题一致性','s1','标题颜色与标准值不一致','是'],['8','文本溢出','s2','文本超出页面边界','否'],['12','字号规范','s3','正文小于 14pt','是'],['14','元素对齐','s3','对象偏离参考线 6pt','是'],['16','空白页面','s3','检测到疑似空白页','否'],['18','安全边距','s4','文字距离页面边缘过近','否']].map((x,i)=>`<tr><td><input type="checkbox" ${i<3?'checked':''}></td><td>${x[0]}</td><td>${x[1]}</td><td><span class="badge ${x[2]}">${x[2].toUpperCase()}</span></td><td>${x[3]}</td><td>${x[4]}</td><td>待处理</td></tr>`).join('')}</tbody></table></div><div class="selectedbar">已选择 3 项　<button class="btn">忽略</button><button class="btn primary" onclick="location.hash='6'">修复选中项</button></div>`),
