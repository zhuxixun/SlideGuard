...
'2':()=>{
  ...
  const qHTML=`<div id="panel-quick" class="rules-panel" style="display:block"><div class="card rules" style="grid-template-columns:1fr"><div class="rule-header" style="font-weight:700;color:#172033;padding-bottom:8px;border-bottom:1px solid var(--line)">快速检查将执行以下 5 项核心规则（固定，不可调整）</div>${quickItems.map(x=>`<div class="rule">${x.n}<span class="muted" style="margin-left:12px">${x.d}</span></div>`).join('')}</div></div>`;
...
/* 04 ── 扫描中 */
'4':()=>shell('扫描结果',`${heading('正在扫描','年度经营汇报.pptx','<button class="btn danger">取消扫描</button>')}<div class="progress-steps">${['解析文件','生成页面预览','执行检查','汇总结果'].map((x,i)=>`<div class="step ${i<2?'done':i===2?'current':''}"><b>${i<2?'✓':i+1}</b>${x}</div>`).join('')}</div><b>正在处理第 18/42 页</b><div class="progress" style="margin:14px 0"><span></span></div><div class="grid scan-cols"><div class="card checklist">${['敏感词检测','字体一致性检查','元素对齐检查','文字安全边距检查','标题一致性检查'].map(x=>`<div class="check">${x}<span class="muted" style="margin-left:auto">已完成</span></div>`).join('')}</div><div class="grid" style="grid-template-columns:1fr 1fr">${[['S1 严重','3','s1'],['S2 高风险','5','s2'],['S3 一般','18','s3'],['S4 建议','2','s4']].map(x=>`<div class="card stat"><span class="badge ${x[2]}">${x[0]}</span><strong>${x[1]}</strong></div>`).join('')}</div></div>`),

/* 04B ── 扫描结果概览 */
'4b':()=>shell('扫描结果',`${heading('扫描结果概览','年度经营汇报.pptx · 标准检查 · builtin-rules-v1.0','<button class="btn" onclick="location.hash=\'2\'">重新扫描</button><button class="btn primary" onclick="location.hash=\'5\'">查看问题</button>')}<div class="grid summary">${[['S1 严重','3','s1'],['S2 高风险','5','s2'],['S3 一般','18','s3'],['S4 建议','2','s4'],['涉及页面','16',''],['可自动修复','11','ok']].map(x=>`<div class="card stat"><span class="badge ${x[2]}">${x[0]}</span><strong>${x[1]}</strong></div>`).join('')}</div><div class="card charts" style="margin-top:20px"><h2>按问题类型统计</h2>${[['敏感词',92],['字体规范',78],['元素对齐',38]].map(x=>`<div class="bar"><label>${x[0]}</label><i style="width:${x[1]}%"></i></div>`).join('')}</div><div class="info" style="margin-top:18px;color:#9b5b00;border-color:#f2c879;background:#fff9e8"><b>扫描未完成</b>　可查看已发现问题，但自动修复不可用。</div>`),

/* 05 ── 问题列表（独立侧边栏页面） */
'5':()=>shell('问题列表',`${heading('问题列表','共 28 个问题','<button class="btn" onclick="location.hash=\'4b\'">返回概览</button>')}<div class="grid filters"><div class="input">搜索问题描述　⌕</div>${['页面','问题类型','严重级别','可自动修复','处理状态'].map(x=>`<div class="input">${x}　⌄</div>`).join('')}</div><div class="card"><table class="table"><thead><tr><th>选择</th><th>页面</th><th>问题类型</th><th>严重级别</th><th>问题描述</th><th>可自动修复</th><th>处理状态</th></tr></thead><tbody>${[['3','敏感词','s1','检测到敏感词：内部代号','否'],['5','字体规范','s1','标题字体不是微软雅黑','是'],['9','标题一致性','s1','标题颜色与标准值不一致','是'],['12','字号规范','s3','正文小于 14pt','是'],['14','元素对齐','s3','对象偏离参考线 6pt','是'],['16','空白页面','s3','检测到疑似空白页','否'],['18','安全边距','s4','文字距离页面边缘过近','否']].map((x,i)=>`<tr class="clickable" onclick="location.hash='6'"><td><input type="checkbox" ${i<3?'checked':''} onclick="event.stopPropagation()"></td><td>${x[0]}</td><td>${x[1]}</td><td><span class="badge ${x[2]}">${x[2].toUpperCase()}</span></td><td>${x[3]}</td><td>${x[4]}</td><td>待处理</td></tr>`).join('')}</tbody></table></div><div class="selectedbar">已选择 3 项　<button class="btn">忽略</button><button class="btn primary" onclick="location.hash='7'">修复选中项</button></div>`),
...
/* 06 ── 问题详情 */
'6':()=>issueDetail({fixable:true}),
'6b':()=>issueDetail({fixable:false}),
'6c':()=>issueDetail({fixable:false,locateFailed:true}),
...
function issueDetail({fixable=true,locateFailed=false}={}){
  const manual=!fixable&&!locateFailed;
  const pages=[3,5,8,9,12,14,16,18,24,31,38,41];
  const thumbnails=pages.map((x,i)=>`<button class="detail-thumb${x===8?' active':''}" aria-label="查看第 ${x} 页"><span class="thumb-canvas"><i></i><i></i><i></i></span><span>第 ${x} 页</span><em class="${i<2?'s1':i<5?'s2':'s3'}">${i%3+1}</em></button>`).join('');
  const preview=locateFailed?`<div class="slide detail-slide unlocated-slide"><div class="locate-empty"><span class="locate-icon">◇</span><b>此页预览不可用于定位</b><span>页面内容未能完整解析，请打开原始 PPT 查看第 8 页。</span></div></div>`:manual?`<div class="slide detail-slide sensitive-slide"><h2>战略合作项目进展</h2><div class="sensitive-hit"><span>敏感词命中</span>内部代号</div></div>`:`<div class="slide detail-slide"><h2>2026 年度经营目标</h2><div class="guide"></div><div class="issuebox"><span>标题文本框</span>聚焦核心业务，持续创新，稳健增长</div><div class="page-issue-dot dot-one">1</div><div class="page-issue-dot dot-two">2</div></div>`;
  const values=locateFailed?`<div class="state-banner unlocated"><b>无法在预览中定位</b><span>检测结论仍可查看，但必须回到原始 PowerPoint 核实页面内容。</span></div><div class="detail-section"><h3>发生了什么</h3><p>第 8 页包含当前预览引擎无法完整解析的对象，因此不能可靠标出具体问题位置。</p></div><div class="detail-section advice"><h3>下一步怎么做</h3><p>在 PowerPoint 中打开第 8 页，检查组合对象、图表或嵌入内容；确认后可忽略本条提示。</p></div>`:manual?`<div class="state-banner manual"><b>需要人工核实</b><span>涉及文本语义，SlideGuard 不会自动删除或替换。</span></div><div class="hit-word"><span>命中词条</span><strong>“内部代号”</strong><small>正文第 1 行 · 第 1–4 个字符</small></div><div class="detail-section"><h3>为什么这样判断</h3><p>该词条存在于你维护的本地敏感词库中，可能不适合出现在对外材料里。</p></div><div class="detail-section advice manual-advice"><h3>建议如何处理</h3><p>结合文档用途核实这段文字；若为对外文件，请手动删除或替换词条。</p></div>`:`<div class="state-banner auto"><b>可安全自动修复</b><span>只调整文本框位置，不改变文字内容。</span></div><div class="value-compare"><div><span>实际位置</span><strong>左侧 2.14 cm</strong></div><b>→</b><div><span>建议位置</span><strong>左侧 1.82 cm</strong></div></div><div class="detail-section"><h3>为什么这样判断</h3><p>同类标题中 86% 已与版式标题参考线对齐，当前标题向右偏离 0.32 cm，超过允许的 3pt 偏差。</p></div><div class="detail-section advice"><h3>建议如何处理</h3><p>将标题文本框向左移动 0.32 cm，使左侧与当前 PPT 版式的标题参考线对齐。</p></div>`;
  const action=fixable?`<div class="detail-actions"><button class="btn">忽略</button><button class="btn primary" onclick="location.hash='7'">修复此问题</button></div>`:`<div class="manual-note"><b>需手动处理</b><span>自动修改可能改变内容语义或版面结构，请按上方建议在 PowerPoint 中处理。</span></div><div class="detail-actions"><button class="btn">忽略</button></div>`;
  const title=locateFailed?'第 8 页预览不完整，无法定位具体对象':manual?'正文包含敏感词“内部代号”':'标题左侧偏离版式参考线 0.32 cm';
  const badge=locateFailed?'<span class="badge s4">S4 提示</span>':manual?'<span class="badge s1">S1 严重</span>':'<span class="badge s3">S3 一般</span>';
  const object=locateFailed?'页面级提示':manual?'正文文本框':'标题文本框';
  const previewFooter=locateFailed?'<div class="page-issue-switch warning-switch"><span>⚠ 无法显示问题标记</span></div>':'<div class="page-issue-switch"><span>本页问题</span><button class="active">1</button><button>2</button></div>';
  return shell('问题详情',`<div class="detail-top"><button class="back-link" onclick="location.hash='5'">← 返回问题列表</button><div class="issue-progress">第 12 / 28 个问题</div><div class="toolbar"><button class="btn">上一个</button><button class="btn">下一个</button></div></div><div class="detail detail-v2"><div class="detail-workspace"><aside class="card detail-pages"><div class="detail-pages-title"><b>问题页面</b><span>12 页</span></div><div class="detail-thumb-list">${thumbnails}</div></aside><section class="card preview detail-preview"><div class="preview-meta"><div><b>第 8 页</b><span>${locateFailed?'预览异常':'本页 2 个问题'}</span></div><span class="preview-note">结构化预览，可能与 PowerPoint 略有差异</span></div><div class="preview-stage ${locateFailed?'stage-unlocated':manual?'stage-manual':''}">${preview}</div>${previewFooter}</section></div><aside class="card panel detail-panel ${locateFailed?'panel-unlocated':manual?'panel-manual':'panel-auto'}"><div class="issue-heading"><div>${badge}<span class="issue-location">第 8 页 · ${object}</span></div><h1>${title}</h1></div>${values}<details class="tech-details"><summary>技术信息</summary><dl><div><dt>规则 ID</dt><dd>${locateFailed?'PREVIEW-01':manual?'R009':'R008'}</dd></div><div><dt>对象 ID</dt><dd>${locateFailed?'不可用':manual?'shape-36':'shape-27'}</dd></div><div><dt>标准来源</dt><dd>${locateFailed?'预览完整性检查':manual?'本地敏感词库':'当前 PPT 版式的标题参考线'}</dd></div><div><dt>处理状态</dt><dd>待处理</dd></div></dl></details>${action}</aside></div>`);
}
